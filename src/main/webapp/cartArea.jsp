...
<div class="right-side-cart-area" id="cartArea">
  ...
  <div class="cart-button">
    <a href="#" id="rightSideCart"><img src="assets/img/core-img/bag.svg" alt=""> <span id="numberProductCart">
      <%if (cart == null) {%>0<%} else {%><%=cart.getNumberProductInCart()%><%}%></span></a>
  </div>
  <div id="listCart">
        ...
        <div class="single-cart-item" id="<%=p.getId_product()%><%=p.getSize()%><%=p.getColor()%>">
          <a href="#" class="product-image">
            ...
            <div class="cart-item-desc">
              <span class="product-remove"><i class="fa fa-close" aria-hidden="true" id="<%=p.getId_product()%><%=p.getSize()%><%=p.getColor()%>" onclick="removeThisProduct(this)"></i></span>
              <span class="badge"><%=p.getProduct_type()%></span>
              <h6><%=p.getProduct_name()%></h6>
              <p class="size">Size: <%=p.getSize()%></p>
              <p class="color">Màu: <%=p.getColor()%></p>
              <p class="color">Số lượng: <%=p.getQuantitySold()%></p>
